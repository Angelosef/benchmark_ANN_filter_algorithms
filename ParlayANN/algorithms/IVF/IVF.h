...
struct IVF_Squared {
  ...
  void fit(PointRange<T, Point> points, csr_filters& filters,
    ...
    this->points = points;
    this->posting_lists =
  parlay::sequence<std::unique_ptr<MatchingPoints<T, Point>>>(filters.n_points);
    ...
    parlay::parallel_for(0, filters.n_points, [&](size_t i) {
      if (filters.point_count(i) >
        ...
        size_t num_clusters = std::max<size_t>(1, filters.point_count(i) / cluster_size);
        this->posting_lists[i] = std::make_unique<PostingListIndex<T, Point>>(
           ...
           KMeansClusterer<T, Point, index_type>(num_clusters),
